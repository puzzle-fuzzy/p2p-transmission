...
只接受完全匹配的版本与字段集合；启动时还会协商文本直达、多人接收和流式恢复能力。协议或能力
不匹配时会停止进入房间并提示刷新升级；旧邀请 query、旧 Cookie、旧本地会话和旧恢复数据库均
不会被读取或迁移。
...
文本直达与 128 KiB DataChannel 文件传输）。`--interop` 让 Firefox/WebKit 建立房间与
