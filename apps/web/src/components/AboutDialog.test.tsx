...
describe('AboutDialog', () => {
  test('renders the product explanation, limits, build information, and no sensitive tokens', () => {
    ...
    expect(screen.getByText(/文件（包括粘贴的文本）会在/)).not.toBeNull()
    expect(screen.getByText(/应用服务器负责/)).not.toBeNull()
    expect(screen.getByText(/接收完成的文件（包括粘贴的文本）会暂存在当前页面/)).not.toBeNull()
