...
M2 建立了不依赖 Bun 生产服务的正式 Rust 2.0 工程：

- 根 Cargo workspace、`Cargo.lock` 与 Rust 1.97.0 固定工具链。
- [`v2/apps/web`](../../../v2/apps/web/Cargo.toml)：Dioxus Web/WASM AppShell。
- [`v2/apps/server`](../../../v2/apps/server/Cargo.toml)：Axum health、build metadata、SPA fallback 与同源静态资源。
...
1.x React/Bun 代码继续保留为产品基线；2.0 位于独立 `v2/` 命名空间，不复用旧 API、协议或数据库。
...
房间表单在 M2 明确显示为未接入，不伪造可用功能。截图见 [`docs/rust-v2`](../../rust-v2/README.md)。后续默认保持这一视觉语言，不进行未经确认的自由改版。
...
完整启动和目录说明见 [`v2/README.md`](../../../v2/README.md)。
