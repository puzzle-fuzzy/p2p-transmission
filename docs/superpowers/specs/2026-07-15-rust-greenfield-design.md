# P2P Transmission：全 Rust Greenfield 重构设计
...
current release 不是把 Bun/Elysia 逐文件翻译成 Rust，也不是让新后端继续服务旧前端。它是一套从零实现的新系统，1.x 只承担三种角色：
...
如果未来需要保留 1.x 房间或数据，应做一次性、离线、显式的数据转换；不能为了一个尚未确认的需求污染 current release 架构。
...
- URL、错误码和服务端事件可以版本化为 current release 专用格式。

### 2.3 明确非目标

- current release 首版不做文件云存储、服务端中转或离线收件箱。
...
flowchart LR
    ...
    UIA <-->|"current release 控制协议"| AX
    UIB <-->|"current release 控制协议"| AX
...
current release 不进行未经确认的视觉改版。默认必须直接延续 1.x 的暗色纯背景、居中窄栏、字号层级、低对比说明文字、紫色强调色、圆角、间距和克制动效。工程基线或中间里程碑也不得用新的营销页、编辑式大标题、网格纹理、装饰光效或双栏仪表盘替代现有产品气质。任何明显视觉方向变化都需要用户明确提出，不能由实现者自由发挥。

## 7. 浏览器平台适配层

`browser-platform` 是 current release 的最高风险区域，必须窄接口封装：
...
- current release 首版失败后“再次发送”重新建立 transfer，不假装支持未验证的断点续传。
...
current release 首版继续使用 SQLite，但采用全新 schema 和 `sqlx` migrations：
...
- 新 schema 不保证 Bun 可读；回滚是回到完整 1.x 部署及其独立数据库，而不是让 Bun 打开 current release 数据库。

### 11.3 1.x 到 current release

推荐不迁移临时房间和匿名会话。发布 current release 时：

- 1.x 在独立路径保留短观察期。
- current release 使用新的数据库文件/volume。
...
- 对 current release 验证同一用户目标，不要求内部网络请求一致。
...
current release 不只比较后端吞吐，还要约束 WebAssembly 用户体验：
...
- SQLite current release 使用独立 volume；coturn 独立进程/容器。
- 1.x 和 current release 首次上线使用独立域名或明确路由，避免共用数据库和 cookie namespace。
...
1. current release 是体验等价的 greenfield rewrite，不是兼容迁移。
...
7. SQLite 使用全新 current release schema；不兼容 1.x 数据库。
