...
- [大文件保存弹层](../../release/screenshots/m7-stream-storage-dialog-desktop-chromium.png)
- [大文件保存完成态](../../release/screenshots/m7-stream-storage-complete-desktop-chromium.png)
...
新增显式慢速门禁 [test_large_file.py](../../../scripts/test_large_file.py)，不进入普通 Playwright 回归。源文件使用磁盘稀疏文件，但在首段、segment 边界、中点和末尾写入不同的非零标记；接收端完成后必须验证准确文件大小和全部标记。
...
python -X utf8 scripts/test_large_file.py --size-gib 1 --profile weak
python -X utf8 scripts/test_large_file.py --size-gib 5 --profile baseline
```
...
- [保存权限失效效果](../../release/screenshots/m8-storage-permission-error-desktop-chromium.png)
- [写入中取消完成态](../../release/screenshots/m8-pending-write-cancelled-desktop-chromium.png)
...
- [磁盘空间不足暂停效果](../../release/screenshots/m8-storage-quota-paused-desktop-chromium.png)
