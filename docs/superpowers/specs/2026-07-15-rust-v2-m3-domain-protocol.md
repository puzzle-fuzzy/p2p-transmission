# Rust 2.0 M3：Domain 与共享协议结果

> 日期：2026-07-15<br>
> 分支：`rust-dev`<br>
> 结论：**M3 通过，可以进入 M4 Axum 2.0 控制面。**
...
黄金样例作为独立 fixture 保存在 `v2/crates/protocol/tests/fixtures`，用于锁定 HTTP、WebSocket 与 DataChannel JSON 的编码结果。
...
python -X utf8 scripts/dev_v2.py --profile release --build-only
python -X utf8 scripts/test_v2_e2e.py
```

最终执行结果：上述工程 gate 全部通过；全工作区 native 共 35 项测试通过，Dioxus WASM release 构建成功，桌面与移动 Chromium 共 2 项 2.0 shell E2E 通过，28 个仓库内文档链接通过检查。

## 6. M4 起点

下一阶段将从全新 SQLite 2.0 schema 与 repository transaction boundary 开始，再把这些状态机接到 Axum HTTP API 和 bounded WebSocket hub。M4 不复用 1.x 数据库、接口或 Bun 服务，也不改变既有前端样式。
