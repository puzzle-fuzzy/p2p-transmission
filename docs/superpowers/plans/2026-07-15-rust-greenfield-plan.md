# P2P Transmission：全 Rust Greenfield 实施计划

> 当前状态（2026-07-16）：current Rust release 已发布并完成生产验收；当前工作重点是发布后稳定性、E2E 门禁和后续性能/平台优化。M7 主线能力、多接收者、5 GiB 流式写盘、同页面与双方刷新恢复、有界重连和最多 10 文件批次已完成，文本/错误矩阵等体验收尾仍可继续。<br>
> 发布质量与生产记录：[Rust 发布手册](../../release/RELEASE.md)、[发布质量审计](../../release/QUALITY-AUDIT.md)<br>
> 工作分支：`main`<br>
> 说明：下方里程碑表保留 2026-07-15 规划时的 gate 快照；当前发布完成情况以发布手册和质量审计为准。<br>
> 设计依据：[全 Rust Greenfield 重构设计](../specs/2026-07-15-rust-greenfield-design.md)
> 验证记录：[Dioxus + Axum + WebRTC Spike 结果](../specs/2026-07-15-dioxus-webrtc-spike-results.md)
> M2 记录：[Rust 工程基线结果](../specs/2026-07-15-rust-m2-engineering-baseline.md)
> M3 记录：[Rust Domain 与协议结果](../specs/2026-07-15-rust-m3-domain-protocol.md)
> M4 记录：[Rust Axum 控制面结果](../specs/2026-07-15-rust-m4-control-plane.md)
> M5 记录：[Rust Dioxus 房间垂直切片结果](../specs/2026-07-15-rust-m5-room-vertical-slice.md)
> M6 记录：[Rust DataChannel 单文件传输结果](../specs/2026-07-15-rust-m6-datachannel-transfer.md)
> M7 阶段记录：[Rust 多接收者传输结果](../specs/2026-07-15-rust-m7-multi-receiver.md)
> M7 大文件与批量记录：[Rust 5 GiB 与最多 10 文件流式传输](../specs/2026-07-15-rust-m7-large-file-protocol.md)
...
- 1.x 与 current release 在迁移期独立构建、独立部署、独立数据库。
- current release 未达到体验等价前，1.x 继续作为可用产品；不让半成品替换线上版本。
...
| M3 ✅ | current release domain/protocol | 状态机和协议不变量全绿 |
| M4 ✅ | current release Axum 控制面 | session/room/join/realtime 可用 |
...
| M9 | current release 独立预发布 | 真实环境观察通过 |
...
- 记录 1.x 用户可见文案和错误映射，current release 可以优化文字，但不能漏掉反馈场景。
...
## 6. M3：current release domain 和共享协议
...
## 7. M4：Axum current release 控制面

### 7.1 SQLite current release
...
- Secure/HttpOnly/SameSite cookie；current release 独立 cookie name。
...
第一次交付端到端可用但尚不完整的 current release：两个浏览器可以创建房间、请求、批准并看到头像入场。
...
- M0 所有核心 Given/When/Then 在 current release 通过。
...
## 12. M9：current release 独立预发布

### 部署方式

- current release 使用独立域名/子域、cookie namespace 和 SQLite volume。
...
- 关键指标稳定且无内存持续增长。
- 回滚只需要入口切回 1.x，不共享 current release 数据库。
- 用户明确确认视觉和功能结果达到 current release 发布标准。

## 13. M10：生产切换与 1.x 退役

### 首次切换

1. 发布 current release 不可变镜像并完成 readiness/smoke。
2. 入口将新访问导向 current release；1.x 使用独立地址保留。
3. 明确告知 1.x 临时房间不能跨到 current release，或等待其自然过期。
4. 观察 room create/join、WS attach、peer connect、transfer completion 和 WebAssembly 加载。
5. 触发停止条件时直接把入口切回 1.x；不转换 current release 数据库给 Bun。
...
1. `docs(rust): define current release greenfield architecture and experience baseline`
...
5. `feat(protocol): define current protocol control and data channel frames`
6. `feat(server): add session room access and sqlite`
...
13. `build(deploy): publish isolated current release release`
...
- current release 独立生产观察期结束后，1.x 才被安全退役。
