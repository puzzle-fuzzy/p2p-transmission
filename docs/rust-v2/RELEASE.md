...
`2.0.0-rc.1` 是独立的 Rust 发布候选版本，不读取 1.x 数据库，也不兼容 1.x API 或实时协议。前端静态资源和 Axum 控制面由同一个容器、正式域名 `https://p2p.yxswy.com` 提供。
...
```bash
python -X utf8 scripts/verify_v2.py
python -X utf8 scripts/test_v2_e2e.py
bun run lint
bun run typecheck
bun run test
bun run build
...
mkdir -p deploy/v2/data
sudo chown 10001:10001 deploy/v2/data
chmod 700 deploy/v2/data
chmod 600 deploy/v2/.env
```
...
正式环境使用 [`p2p.yxswy.com.conf`](../../deploy/v2/nginx/p2p.yxswy.com.conf) 配置 HTTPS/WSS 反向代理。文件正文使用 WebRTC，不需要放大 HTTP 请求体限制；控制 WebSocket 需要保留 Upgrade 头和长连接超时。

`main` 分支的 Rust 2.0 workflow 会在 GitHub Runner 构建不可变镜像，使用 production environment 的 SSH 密钥上传，并通过受限服务器脚本完成 ready 检查和 Nginx 原子切换。首次迁移会保留旧 Bun API/Web 镜像、旧 Nginx 配置、旧 SQLite 数据库和 coturn 容器；失败时自动恢复旧入口。

## 构建、启动与验收

```bash
docker compose --env-file deploy/v2/.env -f deploy/v2/compose.yml config
docker compose --env-file deploy/v2/.env -f deploy/v2/compose.yml build
docker compose --env-file deploy/v2/.env -f deploy/v2/compose.yml up -d
docker compose --env-file deploy/v2/.env -f deploy/v2/compose.yml ps
...
RC1 本地容器验收已经完成：`p2p-transmission-v2:2.0.0-rc.1` 镜像大小为 `12,474,014` 字节，ready 检查返回版本 `2.0.0-rc.1`，Docker health 状态进入 `healthy`。容器内进程确认为 UID/GID `10001`，根文件系统写入失败而 `/app/data` 数据卷可写；收到 Docker SIGTERM 后约 0.35 秒退出，退出码为 0，未发生 OOM。
...
RC1 本地发布门禁在桌面 Chromium 中实际完成了 5 GiB 弱网传输：写入 `5,368,709,120` 字节，按 640 个 8 MiB 段落盘，注入两次 DataChannel 断线后继续，四处确定性内容标记一致。总耗时 917.806 秒，平均 5.579 MiB/s，发送端模拟队列峰值 4,201,088 字节。这个数字用于证明实现边界，不作为公网吞吐承诺。

## 备份、更新与回滚

SQLite 使用 WAL。在线备份优先使用宿主机 `sqlite3` 的 backup 命令：

```bash
mkdir -p deploy/v2/backups
sqlite3 deploy/v2/data/control.sqlite3 ".backup 'deploy/v2/backups/control-$(date +%Y%m%d-%H%M%S).sqlite3'"
```

若宿主机没有 `sqlite3`，先停止容器，再同时复制 `control.sqlite3`、`control.sqlite3-wal` 和 `control.sqlite3-shm`，不要只复制主文件。

更新时使用不可变版本标签，先备份，再构建并替换：
