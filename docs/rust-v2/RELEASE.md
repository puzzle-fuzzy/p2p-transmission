...
`2.0.1` 是当前独立的 Rust 正式版本，不读取 1.x 数据库，也不兼容 1.x API 或实时协议。前端静态资源和 Axum 控制面由同一个容器、正式域名 `https://p2p.yxswy.com` 提供。
...
2.0 发布门禁在桌面 Chromium 中实际完成了 5 GiB 弱网传输：写入 `5,368,709,120` 字节，按 640 个 8 MiB 段落盘，注入两次 DataChannel 断线后继续，四处确定性内容标记一致。总耗时 917.806 秒，平均 5.579 MiB/s，发送端模拟队列峰值 4,201,088 字节。这个数字用于证明实现边界，不作为公网吞吐承诺。
...
正式发布脚本会在加载新镜像前使用 Python `sqlite3.Connection.backup` 创建在线一致性快照，执行 `PRAGMA quick_check`，并在 `deploy/v2/backups` 中保留最近 10 份。备份失败会阻止发布并恢复原环境。
