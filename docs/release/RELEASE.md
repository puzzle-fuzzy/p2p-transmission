...
`main` 分支的生产 workflow 会在 GitHub Runner 构建不可变镜像，使用 production environment 的 SSH 密钥上传，并通过受限服务器脚本完成 ready 检查和 Nginx 原子切换。首次迁移会识别旧 Rust 运行时，迁移其环境、SQLite 数据库和备份，释放旧容器的端口后再切换；失败时自动恢复旧运行时、数据库和入口配置。
