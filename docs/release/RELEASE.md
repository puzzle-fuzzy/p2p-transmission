...
`main` 分支的生产 workflow 会在 GitHub Runner 构建不可变镜像，使用 production environment 的 SSH 密钥上传，并通过受限服务器脚本完成 ready 检查和 Nginx 原子切换。首次迁移会保留旧 Bun API/Web 镜像、旧 Nginx 配置、旧 SQLite 数据库和 coturn 容器；失败时自动恢复旧入口。
