...
mod tests {
    ...
    fn fixture_dir() -> PathBuf {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("p2p-v2-server-{nonce}"));
        fs::create_dir_all(&path).expect("create static fixture directory");
        fs::write(path.join("index.html"), "<main>v2 shell fixture</main>")
    ...
    async fn unknown_application_routes_fall_back_to_the_shell() {
        ...
        let body = response
            ...
            .to_bytes();
        assert!(body.windows(16).any(|window| window == b"v2 shell fixture"));
