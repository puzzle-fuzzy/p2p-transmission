...
components:
  ...
  input-field:
    ...
    padding: "16px"
---
<!-- SEED: 当前为深色主题捕获。计划支持浅色模式，届时更新此文件。 -->
...
- **Ink Muted** (`rgba(255,255,255,0.50)`): 次要文字。文件描述、日志内容、辅助提示。
...
**The Opacity Scale Rule.** 层次不靠阴影，靠白色叠加的透明度阶梯：5% → 10% → 15% → 50% → 80%。每跳对应一个语义角色。不要跳过层级或使用任意值。
...
- **File Item:** 8px 圆角行，白色 5% 透明度底。选中后点击"传输"，行内背景以 Signal Purple 15% 透明度从左向右填充表示进度。完成后文件图标变为紫色对勾。
